...
import {
    Briefcase, MapPin, IndianRupee, Send, FileText, Clock,
    Plus, X, Loader2,
} from "lucide-react";
...
const EditJob = () => {
        ...
        const loadJob = async () => {
            ...
            try {
                ...
                setFormData({
                    ...
                    jobResponsibilities: Array.isArray(job.jobResponsibilities) ? job.jobResponsibilities : [],
    ...
    const validateForm = () => {
        ...
        if (formData.jobDescription.length < 120) return "Description must be at least 120 characters";
        if (formData.requiredSkills.length === 0) return "At least one skill is required";
        if (formData.jobResponsibilities.length === 0) return "At least one responsibility is required";
        if (Number(formData.openings) < 1) return "At least 1 opening is required";

        if (!formData.hideSalary && !formData.salaryNegotiable) {
            if (!formData.salaryMin || !formData.salaryMax) return "Salary range is required";
            if (Number(formData.salaryMax) < Number(formData.salaryMin)) return "Max salary cannot be less than min";
        }

        if (formData.applyType === "email" && !formData.applyEmail) return "Email is required";
        if (formData.applyType === "external-link" && !formData.applyLink) return "Link is required";
    ...
    const handleSubmit = async () => {
        ...
        if (error) {
            Swal.fire({ title: "Validation Error", text: error, icon: "warning", timer: 2000 });
            return;
        }
    ...
    const handleVerifyOtp = async () => {
        ...
        try {
            ...
            Swal.fire("Success!", "Job updated and published successfully.", "success");
        ...
        <div className="max-w-6xl mx-auto py-10 px-4">
            <div className="mb-10 flex items-center justify-between">
                <div>
                    ...
                    <p className="text-gray-600 mt-2">Update the details below and save changes</p>
                </div>
            </div>
                ...
                <div className="p-10 space-y-12">
                    ...
                    <div className="grid md:grid-cols-3 gap-6">
                        <div className="md:col-span-2">
                            <label className="block text-sm font-semibold mb-2">Job Title <span className="text-red-500">*</span></label>
                            <input
                                type="text"
                                name="jobTitle"
                    ...
                    {/* Job Description */}
                    <div>
                        <label className="block text-sm font-semibold mb-2">Job Description <span className="text-red-500">*</span></label>
                        <textarea
                            name="jobDescription"
                    ...
                    <div className="grid lg:grid-cols-2 gap-8">
                        ...
                        <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200">
                            ...
                            <div className="flex gap-3 mb-4">
                                ...
                                />
                                <button onClick={addResponsibility} disabled={!responsibilityInput.trim()} className="px-6 bg-blue-600 hover:bg-blue-700 text-white rounded-xl disabled:opacity-50">
                                    <Plus size={20} />
                                </button>
                            </div>
                            <div className="flex flex-wrap gap-3">
                                {formData.jobResponsibilities.map((item, i) => (
                                    <div key={i} className="bg-white px-4 py-2 rounded-full flex items-center gap-2 border text-sm">
                                        {item}
                                        <button onClick={() => removeResponsibility(item)} className="text-red-500 hover:text-red-600">
                                            <X size={16} />
                                        </button>
                                    </div>
                                ))}
                        ...
                        {/* Required Skills */}
                        <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200">
                            <h3 className="font-semibold text-lg mb-4">Required Skills <span className="text-red-500">*</span></h3>
                            <div className="flex gap-3 mb-4">
                                <input
                                    type="text"
                                    value={skillInput}
                                    onChange={(e) => setSkillInput(e.target.value)}
                                    onKeyDown={(e) => e.key === "Enter" && addItem("requiredSkills", skillInput, setSkillInput)}
                                    placeholder="e.g. React, Node.js"
                                    className="flex-1 px-4 py-3 border border-gray-300 rounded-xl"
                                />
                                <button onClick={() => addItem("requiredSkills", skillInput, setSkillInput)} className="px-6 bg-blue-600 hover:bg-blue-700 text-white rounded-xl">
                                    <Plus size={20} />
                                </button>
                            </div>
                            <div className="flex flex-wrap gap-3">
                                {formData.requiredSkills.map(skill => (
                                    <div key={skill} className="bg-white px-4 py-2 rounded-full flex items-center gap-2 border text-sm">
                                        {skill}
                                        <button onClick={() => removeItem("requiredSkills", skill)} className="text-red-500 hover:text-red-600">
                                            <X size={16} />
                                        </button>
                                    </div>
                                ))}
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                        <div>
                            ...
                            <select value={formData.jobType} onChange={(e) => handleSelectChange("jobType")(e.target.value)} className="w-full px-5 py-3.5 border rounded-2xl">
                        ...
                        <div>
                            ...
                            <select value={formData.workMode} onChange={(e) => handleSelectChange("workMode")(e.target.value)} className="w-full px-5 py-3.5 border rounded-2xl">
                        ...
                        <div>
                            ...
                            <select value={formData.shiftType} onChange={(e) => handleSelectChange("shiftType")(e.target.value)} className="w-full px-5 py-3.5 border rounded-2xl">
                                {TIMINGS.map(t => <option key={t} value={t.toLowerCase().replace(" ", "-")}>{t}</option>)}
                            </select>
                        </div>

                        <div>
                            <label className="block text-sm font-medium mb-2">No. of Openings <span className="text-red-500">*</span></label>
                            <input type="number" name="openings" value={formData.openings} onChange={handleInputChange} min="1" className="w-full px-5 py-3.5 border rounded-2xl" />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="grid md:grid-cols-4 gap-6">
                            <div>
                                ...
                                <input name="country" value={formData.country} onChange={handleInputChange} className="w-full px-5 py-3.5 border rounded-2xl" />
                            </div>
                            <div>
                                <label className="block text-sm mb-2">State</label>
                                <input name="state" value={formData.state} onChange={handleInputChange} className="w-full px-5 py-3.5 border rounded-2xl" />
                            </div>
                            <div>
                                <label className="block text-sm mb-2">City</label>
                                <input name="city" value={formData.city} onChange={handleInputChange} className="w-full px-5 py-3.5 border rounded-2xl" />
                            </div>
                            <div>
                                <label className="block text-sm mb-2">Display Location</label>
                                <input name="locationText" value={formData.locationText} onChange={handleInputChange} placeholder="e.g. Koramangala, Bengaluru" className="w-full px-5 py-3.5 border rounded-2xl" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="flex flex-wrap gap-6 mb-6">
                            <label className="flex items-center gap-2">
                                <input type="checkbox" checked={formData.hideSalary} onChange={(e) => setFormData(p => ({ ...p, hideSalary: e.target.checked, salaryNegotiable: false }))} />
                                Hide Salary Range
                            </label>
                            <label className="flex items-center gap-2">
                        ...
                        {!formData.hideSalary && !formData.salaryNegotiable && (
                            <div className="grid md:grid-cols-4 gap-6">
                                <div>
                                    ...
                                    <select value={formData.currency} onChange={(e) => handleSelectChange("currency")(e.target.value)} className="w-full px-5 py-3.5 border rounded-2xl">
                                ...
                                <div>
                                    ...
                                    <input type="number" name="salaryMin" value={formData.salaryMin} onChange={handleInputChange} className="w-full px-5 py-3.5 border rounded-2xl" />
                                </div>
                                <div>
                                    <label className="block text-sm mb-2">Max Salary</label>
                                    <input type="number" name="salaryMax" value={formData.salaryMax} onChange={handleInputChange} className="w-full px-5 py-3.5 border rounded-2xl" />
                                </div>
                                <div>
                                    <label className="block text-sm mb-2">Salary Type</label>
                                    <select value={formData.salaryType} onChange={(e) => handleSelectChange("salaryType")(e.target.value)} className="w-full px-5 py-3.5 border rounded-2xl">
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                        <div>
                            <label className="block text-sm mb-2">Min Experience (years)</label>
                            <input type="number" name="experienceMin" value={formData.experienceMin} onChange={handleInputChange} className="w-full px-5 py-3.5 border rounded-2xl" />
                        </div>
                        <div>
                            <label className="block text-sm mb-2">Max Experience (years)</label>
                            <input type="number" name="experienceMax" value={formData.experienceMax} onChange={handleInputChange} className="w-full px-5 py-3.5 border rounded-2xl" />
                        </div>
                        <div>
                            <label className="block text-sm mb-2">Notice Period</label>
                            <input name="noticePeriod" value={formData.noticePeriod} onChange={handleInputChange} placeholder="30 days / Immediate" className="w-full px-5 py-3.5 border rounded-2xl" />
                        </div>
                        <div>
                            <label className="block text-sm mb-2">Education</label>
                            <select value={formData.education} onChange={(e) => handleSelectChange("education")(e.target.value)} className="w-full px-5 py-3.5 border rounded-2xl">
                        ...
                        <div className="flex flex-wrap gap-3">
                            {COMMON_BENEFITS.map(benefit => (
                                <button
                                    ...
                                    className={`px-5 py-2 rounded-full text-sm transition-all ${formData.benefits.includes(benefit)
                    ...
                    <div>
                        ...
                        {formData.applyType === "external-link" && (
                            <div className="mt-6">
                                <label className="block text-sm mb-2">External Application Link</label>
                                <input name="applyLink" value={formData.applyLink} onChange={handleInputChange} placeholder="https://" className="w-full px-5 py-3.5 border rounded-2xl" />
                            </div>
                        )}
                        ...
                        {formData.applyType === "email" && (
                            <div className="mt-6">
                                ...
                                <input name="applyEmail" value={formData.applyEmail} onChange={handleInputChange} placeholder="careers@company.com" className="w-full px-5 py-3.5 border rounded-2xl" />
                            </div>
                        )}
                    </div>
            ...
            {showOtpModal && (
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-3xl p-8 max-w-md w-full">
                        ...
                        <p className="text-gray-600 mb-8">Enter the 6-digit OTP sent to your registered email</p>

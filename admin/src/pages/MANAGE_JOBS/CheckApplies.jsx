  ...
  return (
    <div className="relative">
      <button
        ...
        disabled={loading}
        className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-violet-600 hover:bg-violet-700 disabled:opacity-50 rounded-xl shadow-sm transition-colors"
      >
        <Download size={13} />
          ...
          <div className="absolute right-0 top-full mt-1.5 w-44 bg-white rounded-xl border border-gray-200 shadow-lg z-20 overflow-hidden">
            ...
            >
              ...
              <div className="text-left">
                ...
                <p className="text-xs text-gray-400">With summary sheet</p>
              </div>
            </button>
            <button
              onClick={() => {
            ...
            >
              ...
              <div className="text-left">
                ...
                <p className="text-xs text-gray-400">Raw data only</p>
              </div>
            </button>
          </div>
        </>
...
const CheckApplies = () => {
  ...
  const handleExport = async (format = "xlsx") => {
    try {
      ...
      const res = await api.get(`/applications/export/${jobId}`, {
        params: {
          status: statusFilter || undefined,
          search: search || undefined,
          dateFrom: dateRange.from || undefined,
      ...
      link.href = url;
      link.download = `applications_${jobId}_${Date.now()}.${format}`;
        ...
        await api.post(`/applications/create-interview/${selectedApp.id}`, {
          ...
          meetingLink:
            createForm.interviewType === "online"
              ...
              : null,
          location:
            createForm.interviewType === "offline" ? createForm.location : null,
  ...
  const stats = {
    ...
    selected: applications.filter(
      (a) => a.status === "selected" || a.isSelected,
    ).length,
    rejected: applications.filter((a) => a.status === "rejected").length,
    shortlisted: applications.filter((a) => a.status === "shortlisted").length,
    interview: applications.filter((a) => a.status === "interview_stage")
      .length,
    finalShortlist: applications.filter((a) => a.status === "final_shortlist")
      .length,
    hold: applications.filter((a) =>
      a.interviews?.some((i) => i.status === "hold"),
    ).length,
    offerSent: applications.filter((a) => a.offerEmailSent).length,
    joined: applications.filter((a) => a.status === "joined").length,
    not_joined: applications.filter((a) => a.status === "not_joined").length,
  };
            ...
            <div className="border-t border-gray-100 px-4 py-4 bg-gray-50/50 space-y-5">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {[
                    ...
                    <button
                      key={opt.value}
                      onClick={() => setInterviewStatusFilter(opt.value)}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {[
                    ...
                    <button
                      key={opt.value}
                      onClick={() => setInterviewResultFilter(opt.value)}
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <input
                    type="text"
                    placeholder="Area (e.g. Sector 15)"
                    value={candidateArea}
                    onChange={(e) => {
                      setCandidateArea(e.target.value);
                      setPage(1);
                    }}
                    className="flex-1 text-sm px-3 py-2 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-violet-500 bg-white"
                  />
        ...
        {!loading && applications.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3 mb-5">
            {[
              ...
              <div
                key={label}
                className="bg-white rounded-xl border border-gray-100 px-3 py-2.5 text-center"
              >
                ...
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50/50">
                    {[
                      ...
                      <th
                        key={h}
                        className={`px-5 py-3.5 text-xs font-semibold text-gray-400 uppercase tracking-wider ${i === 6 ? "text-right" : "text-left"}`}
                      >
                        {h}
                      </th>
                    ))}
              ...
              <div className="space-y-4">
                ...
                {createForm.interviewType === "phone" && (
                  <div className="flex items-center gap-2 px-3.5 py-3 bg-blue-50 border border-blue-100 rounded-xl text-xs text-blue-700">
                    <Phone size={14} className="shrink-0" /> Phone interview —
                    no meeting link or location required.
                  </div>
                )}
                    ...
                    {[
                      {
                        value: "scheduled",
                        label: "Scheduled",
                        selectedCls: "border-blue-500 bg-blue-50 text-blue-700",
                      },
                      {
                        value: "in_progress",
                        label: "In Progress",
                      ...
                      },
                      {
                        value: "cancelled",
                        label: "Cancelled",
                        selectedCls: "border-red-500 bg-red-50 text-red-700",
                      },
                      {
                        value: "rescheduled",
                        label: "Reschedule",
                      ...
                      >
                        ...
                        {item.options?.length > 0 && (
                          <div className="flex flex-wrap gap-1.5 mb-2">
                            {item.options.map((opt, oIdx) => (
                              <span
                                key={oIdx}
                                className={`px-2.5 py-1 rounded-lg text-xs font-medium border ${opt === item.answer ? "bg-teal-600 text-white border-teal-600" : "bg-white text-gray-500 border-gray-200"}`}
                              >
                                {opt === item.answer && (
                        ...
                        {item.type !== "text" && (
                          <p className="text-xs text-gray-400 mt-1">
                            Answer:{" "}
                            <span
                              className={`font-semibold ${item.answer === "Yes" ? "text-green-600" : item.answer === "No" ? "text-red-500" : "text-gray-700"}`}
                            >
                              {item.answer}
                            </span>
                          </p>
